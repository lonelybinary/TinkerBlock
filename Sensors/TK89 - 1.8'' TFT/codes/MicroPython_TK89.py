"""
TK89-1.8INCH TFT ST7735 - Raspberry Pi Pico 2 Example

Description:
This script demonstrates how to use the TK89-1.8INCH TFT ST7735 module
...
Pin Connections (Raspberry Pi Pico 2):
TFT screen module → Raspberry Pi Pico 2
- GND → GND (ground)
- 3V3 → 3.3V (power supply, Pico can directly provide 3.3V)
- CS → GPIO 5 (chip select pin, can be modified)
- DC → GPIO 4 (data/command selection pin, can be modified)
- RST → GPIO 3 (reset pin, can be modified)
- BL → GPIO 2 (backlight control pin, can be modified)
- SCL/SCK → GPIO 18 (SPI clock, hardware SPI)
- SDA/MOSI → GPIO 19 (SPI data, hardware SPI)

Note:
- Pico is a 3.3V MCU, can connect directly, no level conversion needed
- SPI pins (SCK and MOSI) must use hardware SPI pins
- Other pins (CS, DC, RST, BL) can be modified to other GPIO pins as needed

Usage:
1. First save st7735.py library file to Pico (in Thonny, open st7735.py → Save As → MicroPython Device)
2. Save this script to Raspberry Pi Pico 2
3. Run the script and check terminal output
4. Observe various test effects on screen

Note: st7735.py library file is included in this folder, need to save to Pico first
"""
