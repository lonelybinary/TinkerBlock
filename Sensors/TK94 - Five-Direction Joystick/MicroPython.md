...
This example shows how to use the TK94 - FIVE-DIRECTION JOYSTICK module on a Raspberry Pi Pico 2 to detect joystick direction.
...
![Effect](images/TK94-pico-uart.png)
...
- **`SIGNAL_PIN`:** GPIO number for five-direction joystick SIGNAL (must be ADC pin). Change this if you use another pin.
...
- **`ADC(Pin(SIGNAL_PIN))`:** Create ADC object for reading analog signals.

**Lines 13–44: Main loop**
...
- **`signal.read_u16()`:** Read ADC analog value (0-65535).
- **`if signal_value >= 6400 and signal_value <= 8320`:** Check if joystick is pushed up, analog value in range 6400-8320.
- **`elif signal_value >= 17280 and signal_value <= 18560`:** Check if joystick is pushed left, analog value in range 17280-18560.
- **`elif signal_value >= 28800 and signal_value <= 30080`:** Check if joystick is pushed down, analog value in range 28800-30080.
- **`elif signal_value >= 64000 and signal_value <= 65535`:** Check if joystick is pushed right, analog value in range 64000-65535.
- **`elif signal_value >= 11520 and signal_value <= 12160`:** Check if joystick is pressed, analog value in range 11520-12160.
- **`else`:** Other cases indicate joystick is in center position.
- **`print(f"...")`:** Print joystick direction and current analog value to terminal.
- **`time.sleep(0.2)`:** Wait 200 milliseconds before reading again to control detection frequency.
