...
This example shows how to use the TK18 - MAGNETIC HALL SENSOR module on a Raspberry Pi Pico 2 to detect magnetic fields.
...
HALL_PIN = 0    # GPIO connected to SIGNAL (e.g. GPIO 0)
LED_PIN = 1     # LED pin (GPIO 1, or external LED, Pico built-in LED is GPIO 25)

# Create pin objects
hall = Pin(HALL_PIN, Pin.IN)   # Set Hall sensor pin as input (to read detection state)
led = Pin(LED_PIN, Pin.OUT)    # Set LED pin as output (to control LED on/off)

print("Magnetic Hall sensor program started")
print("LED on when magnetic field detected, LED off when no magnetic field")

# Main loop: runs forever
while True:
    # Read Hall sensor state
    hallState = hall.value()   # Read Hall sensor pin level: 1 = magnetic field detected (HIGH), 0 = no magnetic field (LOW)
...
- **`HALL_PIN = 0`:** GPIO number for Hall sensor SIGNAL. Change this if you use another pin.
...
- **`Pin(HALL_PIN, Pin.IN)`:** Set Hall sensor pin as input to read detection state.
...
- **`hall.value()`:** Read Hall sensor pin level, returns 1 when magnetic field is detected (HIGH), 0 when no magnetic field (LOW).
- **`if hallState == 1`:** Check if magnetic field is detected; if detected, execute LED on operation.
- **`led.on()`:** Output HIGH, LED on.
- **`led.off()`:** Output LOW, LED off.
